#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the zeropowers plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/zeropowers.js"
...
skill_count=$(find "$ZEROPOWERS_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $ZEROPOWERS_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-zeropowers skill exists (critical for bootstrap)
echo "Test 3: Checking using-zeropowers skill (required for bootstrap)..."
if [ -f "$ZEROPOWERS_SKILLS_DIR/using-zeropowers/SKILL.md" ]; then
    echo "  [PASS] using-zeropowers skill exists"
else
    echo "  [FAIL] using-zeropowers skill not found (required for bootstrap)"
    exit 1
fi
...
if grep -q 'configDir}/skills/zeropowers/' "$SUPERPOWERS_PLUGIN_FILE"; then
