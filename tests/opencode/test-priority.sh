...
# Verifies that skills are resolved with correct priority: project > personal > zeropowers
...
# 1. Create in zeropowers location (lowest priority)
mkdir -p "$ZEROPOWERS_SKILLS_DIR/priority-test"
cat > "$ZEROPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
...
if [ -f "$ZEROPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
...
# Test 2: Test that personal overrides zeropowers
echo ""
echo "Test 2: Testing personal > zeropowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides zeropowers)"
    ...
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|zeropowers" | head -10
fi

# Test 3: Test that project overrides both personal and zeropowers
echo ""
echo "Test 3: Testing project > personal > zeropowers priority..."
...
# Test 4: Test explicit zeropowers: prefix bypasses priority
echo ""
echo "Test 4: Testing zeropowers: prefix forces zeropowers version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load zeropowers:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [PASS] zeropowers: prefix correctly forces zeropowers version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] zeropowers: prefix did not force zeropowers version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
